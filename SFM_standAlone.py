...
def visualize(prev_container , curr_container ,focal , pp , prev_frame_id , curr_frame_id ):
    ...
    plt.imshow(prev_container.img)
    prev_p = prev_container.traffic_light
    prev_p=np.array(prev_p)
    plt.plot(prev_p[:,0], prev_p[:,1], 'b+')

    # plt.set_title('curr(' + str(curr_frame_id) + ')')
    plt.imshow(curr_container.img)
    curr_p = np.array(curr_container.traffic_light)
    plt.plot(curr_p[:,0], curr_p[:,1], 'b+')

    for i in range(len(curr_p)):
        plt.plot([curr_p[i,0], foe[0]], [curr_p[i,1], foe[1]], 'b')
        if curr_container.valid[i]:
            plt.text(curr_p[i,0], curr_p[i,1], r'{0:.1f}'.format(curr_container.traffic_lights_3d_location[i, 2]), color='r')
    plt.plot(foe[0], foe[1], 'r+')
    plt.plot(rot_pts[:,0], rot_pts[:,1], 'g+')
